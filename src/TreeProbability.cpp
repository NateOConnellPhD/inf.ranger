...
bool TreeProbability::findBestSplit(size_t nodeID, std::vector<size_t>& possible_split_varIDs) {
  ...
  if (penalize_split_competition) {
    ...
    // Pass 1: Find best split for each variable
    ...
    // Pass 2: Apply standardized criterion
    ...
    for (size_t k = 0; k < var_ids.size(); ++k) {
      ...
      if (n_left_star == 0 || n_right_star == 0) continue;

      // Balance correction + standardized criterion
  ...
  // For all possible split variables (STANDARD PATH)
  for (auto& varID : possible_split_varIDs) {
  ...
  } // end penalize_split_competition
