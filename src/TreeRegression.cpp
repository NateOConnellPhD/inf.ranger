...
bool TreeRegression::findBestSplit(size_t nodeID, std::vector<size_t>& possible_split_varIDs) {
  ...
  if (num_samples_node >= 2 * (*min_bucket)[0]) {
    ...
    if (penalize_split_competition) {
      ...
      //      (n_L*n_R/n_t ~ n_t/4), while binary variables at low prevalence are
      ...
      for (size_t pos = start_pos[nodeID]; pos < end_pos[nodeID]; ++pos) {
        size_t sampleID = sampleIDs[pos];
        double diff = data->get_y(sampleID, 0) - mean_node;
        ss += diff * diff;
      }
      ...
      std::vector<double> var_decreases;  // raw ranger decrease = sum_L^2/n_L + sum_R^2/n_R
      std::vector<double> var_values;
      std::vector<size_t> var_num_cutpoints;
      ...
      var_num_cutpoints.reserve(possible_split_varIDs.size());

      // Pass 1: Find best split for each variable independently
      for (auto& varID : possible_split_varIDs) {
        ...
        if (data->isOrderedVariable(varID)) {
          if (memory_saving_splitting) {
            ...
            data->getAllValues(tmp_vals, sampleIDs, varID, start_pos[nodeID], end_pos[nodeID]);
            num_cutpoints = (tmp_vals.size() > 1) ? tmp_vals.size() - 1 : 1;
          } else {
            ...
            if (q < Q_THRESHOLD) {
              ...
              num_cutpoints = data->getNumUniqueDataValues(varID);
              if (num_cutpoints > 1) --num_cutpoints;
            }
          }
        } else {
          findBestSplitValueUnordered(nodeID, varID, sum_node, num_samples_node,
              var_best_value, var_best_varID, var_best_decrease);
          num_cutpoints = data->getNumUniqueDataValues(varID);
          if (num_cutpoints < 1) num_cutpoints = 1;
        }
        ...
        if (var_best_decrease >= 0) {
          ...
          var_num_cutpoints.push_back(num_cutpoints);
        }
      }

      // Pass 2: Apply standardized criterion and select winner
      ...
      for (size_t k = 0; k < var_ids.size(); ++k) {
        // Extract CART criterion: G_j* = (n_L*n_R/n_t) * (ybar_L - ybar_R)^2
        // From ranger's decrease = sum_L^2/n_L + sum_R^2/n_R = G_j* + sum_node^2/n_t
        ...
        size_t varID_k = var_ids[k];
        double split_val_k = var_values[k];

        if (data->isOrderedVariable(varID_k)) {
          // Ordered: count observations with x <= split_value
          for (size_t pos = start_pos[nodeID]; pos < end_pos[nodeID]; ++pos) {
            size_t sampleID = sampleIDs[pos];
            if (data->get_x(sampleID, varID_k) <= split_val_k) {
              ++n_left_star;
            }
          }
        } else {
          // Unordered factor: split_value is bitmask splitID
          // Right child if bit at factorID position is set
          size_t splitID = (size_t) split_val_k;
          size_t n_right_count = 0;
          for (size_t pos = start_pos[nodeID]; pos < end_pos[nodeID]; ++pos) {
            size_t sampleID = sampleIDs[pos];
            double value = data->get_x(sampleID, varID_k);
            size_t factorID = std::floor(value) - 1;
            if (splitID & (1ULL << factorID)) {
              ++n_right_count;
            }
          }
          n_left_star = num_samples_node - n_right_count;
        }

        size_t n_right_star = num_samples_node - n_left_star;

        // Guard against degenerate splits (shouldn't happen since G_j > 0)
        if (n_left_star == 0 || n_right_star == 0) {
          continue;
        }

        // Balance correction: multiply by n_t / (n_L* * n_R*) to remove balance weight
        // This converts G_j = (n_L*n_R/n_t)*(ybar_L-ybar_R)^2 to (ybar_L-ybar_R)^2
        // i.e., pure separation on the mean scale
        double balance_factor = (double) num_samples_node / ((double) n_left_star * (double) n_right_star);
        double standardized = balance_factor * G_j_penalized;

        viable_idx.push_back(k);
        standardized_values.push_back(standardized);
      }

      // Select winner: softmax (proportional) or argmax (standard)
      if (!viable_idx.empty()) {
        size_t winner;

        if (softmax_split && viable_idx.size() > 1) {
          // Softmax: draw variable with probability proportional to standardized impurity
          double total = 0.0;
          for (auto& s : standardized_values) total += s;
          std::uniform_real_distribution<double> udist(0.0, 1.0);
          double u = udist(random_number_generator);
          double cumprob = 0.0;
          winner = viable_idx.size() - 1;  // default to last
          for (size_t j = 0; j < viable_idx.size(); ++j) {
            cumprob += standardized_values[j] / total;
            if (u <= cumprob) {
              winner = j;
              break;
            }
          ...
          // Argmax: select variable with highest standardized criterion
          winner = std::distance(standardized_values.begin(),
                     std::max_element(standardized_values.begin(), standardized_values.end()));
        }
      ...
      // === STANDARD SPLITTING (original code) ===
      for (auto& varID : possible_split_varIDs) {
        if (data->isOrderedVariable(varID)) {
          if (memory_saving_splitting) {
